...
&emsp;&emsp;点击上方“内容查询”按钮，可以进行简单的内容查询。您可以选择查询的条件，也可以选择查询结果排序的方式。

&emsp;&emsp;点击上方“复合查询”按钮，可以进行多条件的复合查询。您可以选择两个条件进行查询。

&emsp;&emsp;点击上方“热度分析”按钮，可以进行折线图的生成。输入一个关键词后，该折线图可以为您描绘该关键词每一天出现的数量。

&emsp;&emsp;点击上方“词云展示”按钮，可以进行词云图的生成。您可以得到一张所有关键词的词云图。

&emsp;&emsp;点击上方“留言板块”按钮，您可以对该网站做出评价。

&emsp;&emsp;点击上方“主页引导”按钮，您可以回到主页面。

## 网页演示.mp4
&emsp;&emsp;**具体展示了网页各功能的使用。** 如果想要知道网页最后的完成效果与功能，以及试图复现网页中的功能，可以观看该视频。视频时长1分钟41秒。

&emsp;&emsp;**最终版本的网页外观与视频中的有出入（如词云图形状，页面底部版权声明等），功能上也有所增加（新增“留言板块”），但其余部分功能性上是一致的。如果想对外观进行更多观察，请以利用最新代码生成的网页为准。**
...
9. **基于 Echarts 绘制了折线图，鼠标悬停可以显示具体数值。**
10. **基于 Echarts 绘制了词云图，鼠标悬停可以显示具体数值，自定义了词云图的图案。**
11. **允许了用户提交反馈，服务端可以从本地文件中得到反馈的文本信息。**
...
* 网站设立了反爬机制。**对于部分的反爬网站，我们可以通过减慢爬虫的速度（如每1秒才读取1条新闻数据）来解决该问题。**
...
&emsp;&emsp;这个问题虽然是一个小问题，小到在进行课程汇报的时候可以无视它的存在，但必须承认它的存在让这个功能不太完美。经过研究，我发现可能是因为，在执行让词云图出现在页面上的方法时，词云图的渲染并没有执行结束。意即，在生成正确的词云图之前，我就已经希望词云图显示在页面上了。这样做的后果就是页面上什么都不显示。

&emsp;&emsp;最终，我发现调用 setTimeout 函数可以将图像显示方法推迟到下一个事件循环中执行，确保了在 DOM（文档对象模型）更新后再显示词云图，解决了问题。

5. 将用户的反馈内容存储在本地文件障碍重重

&emsp;&emsp;原本我猜测，可以通过直接在 HTML 文件中的 script 部分将读取到的数据写入到本地文件中，但事实上的操作比我想象的要复杂。如果 HTML 允许用户在任何可以提交内容的网页中将数据传入到本地文件，那么安全性将备受质疑，因此这个方案是不能直接实现的。

&emsp;&emsp;经过研究，我发现可以通过将在前端读取到的数据传入后端，由后端将数据写入到本地文件中。于是在这里，我使用了与之前发送给后端的 GET 请求不同的 POST 请求，发送了表单给后端进行处理。最终，用户反馈内容成功被存储在了本地文件。

6. 自定义词云图背景时，总是没有得到最好的效果。

&emsp;&emsp;加入自定义背景并不是像制作简单词云图那样选择一个形状就可以解决的事情。实际上，我们需要选择一张非常合适的背景图片，将其转化为编码的方式让 Echarts 去识别。这期间，我尝试过各种不同的图片，并且往往是以词云图不显示或实现效果不佳告终的。

&emsp;&emsp;最终，我找到了一张填充较好的 Twitter 的 Logo，将其转化为 base64 编码让 Echarts 识别，并最终得到了实现效果较好的词云图。

7. 缺少对于 MySQL, CSS, HTML 语法的认识
